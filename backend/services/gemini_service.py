...
def _build_schema(include_food: bool) -> dict:
    properties = {
    ...
    required = ["weather_desc", "spot_reason", "spot_section_title", "hidden_spots"]

    if include_food:
        properties["food_section_title"] = {"type": "string"}
        properties["local_restaurants"] = {
            "type": "array",
            "items": {
                "type": "object",
                "properties": {
        ...
        required += ["food_section_title", "local_restaurants"]
...
def _build_prompt(region: str, weather: dict, count: int, interests: list[str], include_food: bool) -> str:
    interest_line = ""
    title_guide = '- spot_section_title: 명소 섹션의 제목을 지으세요 (기본값 예시: "숨겨진 현지 명소").\n'
    if include_food:
        title_guide += '- food_section_title: 맛집 섹션의 제목을 지으세요 (기본값 예시: "현지인 숨은 맛집").\n'

    if interests:
        interest_list = ", ".join(interests)
        interest_line = f"\n사용자가 선택한 주요 관심사: {interest_list}\n- 위 관심사와 관련된 장소{'·맛집' if include_food else ''} 위주로 추천하세요.\n"
        title_guide = (
    ...
    food_requirement = '- 현지인이 즐겨 찾는 "숨은 맛집" {count}곳을 추천하세요.\n'.format(count=count) if include_food else ""
...
{food_requirement}- 각 추천마다 위 날씨 조건에서 왜 그 장소가 적합한지 이유를 함께 제시하세요 (예: 비/폭염이면 실내·그늘 위주, 맑고 선선하면 야외 위주).
"""


def generate_recommendations(region: str, weather: dict, count: int = 3, interests: list[str] | None = None) -> dict:
    interests = interests or []
    include_food = "음식" in interests
    ...
    response = client.models.generate_content(
        model=model,
        contents=_build_prompt(region, weather, count, interests, include_food),
        config=types.GenerateContentConfig(
            response_mime_type="application/json",
            response_schema=_build_schema(include_food),
        ),
    )
